...
import (
	...
	trackingserverclient "github.com/BernsteinMond/gorecengine/inference/src/internal/external/tracking/client"
...
func run() (err error) {
	...
	trackingServerCl := trackingserverclient.New()

	service := core.New(
		trackingServerCl,
	)

	srv := httpserver.New(cfg.Port, service)
	...
	go func(ctx context.Context) {
		defer stopWg.Done()
		httpSrvErr := launchHTTPServer(ctx, srv)
		if httpSrvErr != nil {
